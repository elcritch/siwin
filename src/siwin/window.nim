...
when siwin_build_lib:
  ...
  proc siwin_window_service(window: Window) =
    window.serviceWindow()

  proc siwin_window_run(window: Window, makeVisible: cchar) =
    window.run(makeVisible.bool)

  proc siwin_poll_events(globals: SiwinGlobals): cchar =
    globals.pollEvents().cchar
  ...
  proc siwin_wake_event_loop(globals: SiwinGlobals) =
    globals.wakeEventLoop()

  proc siwin_event_loop_waker(globals: SiwinGlobals): CEventLoopWakerHandle =
    result = cast[CEventLoopWakerHandle](allocShared0(sizeof(CEventLoopWakerHandleObj)))
  ...
  proc siwin_destroy_event_loop_waker(handle: CEventLoopWakerHandle) =
    if handle != nil:
