...
const siwin_use_wayland* {.booldefine: "siwin.wayland".} =
...
proc siwin_default_platform(): Platform {.siwin_import_export.} =
  defaultPreferedPlatform()

proc defaultPreferedPlatform*(): Platform {.siwin_export_import.} =
  siwin_default_platform()
...
proc platformToUse*(available: seq[Platform], prefered: Platform): Platform =
  if prefered in available:
    ...
    raise
      SiwinPlatformSupportDefect.newException("No platforms available to open a window")

when not siwin_use_lib:
  proc newSiwinGlobals*(
...
proc newSiwinGlobals*(
    preferedPlatform: Platform = defaultPreferedPlatform()
): SiwinGlobals {.siwin_export_import.} =
  siwin_new_globals(preferedPlatform)

when siwin_build_lib:
  {.push exportc, cdecl, dynlib.}
  proc siwin_destroy_globals(globals: SiwinGlobals) {.nodestroy.} =
    GC_unref(globals)
